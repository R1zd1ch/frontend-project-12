...
test.describe('registration', () => {
  ...
  test('handle validation', async ({ page }) => {
    ...
    await expect(await page.locator('text=Пароли должны совпадать')).toHaveCount(1);
  });
});
...
test.describe('auth', () => {
  ...
  test('successful login', async ({ page }) => {
    ...
    await expect(await page.locator('text=Неверные имя пользователя или пароль')).toHaveCount(0);
  });
  ...
  test('handle login error', async ({ page }) => {
    ...
    await expect(await page.locator('text=Неверные имя пользователя или пароль')).toHaveCount(1);
  });
});
...
test.describe('chat', () => {
  ...
  test('profanity filter', async ({ page }) => {
    ...
    await expect(await page.locator('text=you have nice *****')).not.toHaveCount(0);
  ...
  test('different channels', async ({ page }) => {
    ...
    await page.keyboard.press('Enter');
    await expect(await page.locator('text=message for general')).not.toHaveCount(0);
    ...
    await page.keyboard.press('Enter');
    await expect(await page.locator('text=message for random')).not.toHaveCount(0);
  });
...
test.describe('two users chatting', () => {
  ...
  test('login second user', async () => {
    await expect(await page2.locator('text=Неверные имя пользователя или пароль')).toHaveCount(0);
  });
  ...
  test('only initiator is switched to new channel', async ({ page }) => {
    ...
    await expect(await page.getByRole('button', { name: 'test channel 2' })).toHaveClass(
      'w-100 rounded-0 text-start text-truncate btn btn-secondary',
    );
    await expect(await page2.getByRole('button', { name: 'test channel 2' })).not.toHaveClass(
      'w-100 rounded-0 text-start text-truncate btn btn-secondary',
    );
  });
});
